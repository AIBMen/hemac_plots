...
from utils import (LMLABEL_HEAD, LMLABEL_ADMISSIONS, LMLABEL_ENROLL_UNDERGRAD, LMLABEL_ENROLL_GRAD, 
                   LMLABEL_GRADUATION_ASSC, LMLABEL_GRADUATION_BACH)
...
class LandingMap:
    ...
    def __init__(self,
        ...
        '''Build HEMAC Landing Page Map with Folium
        ...
        for schl in self.schools:
            ...
            mkr = folium.Marker(
                ...
                tooltip=folium.Tooltip(text=f"<b>{dat['name']}</b><br>({dat['city']}, {dat['state']})",
                                       style='color:#06474D;;font-family:Georgia, serif;font-size:13px;text-align:center;'),
                icon=folium.plugins.BeautifyIcon(icon_shape='marker',
                                       icon='institution',
                                       text_color="white",
                                       border_width=0,
                                       background_color="#06474D"),
                name=dat['name']
            )
            mkr.add_to(fg)
        self.map.save('docs/landing_map.html')
